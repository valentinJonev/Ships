...
gulp.task('configure', function () {
    gulp.src('config.json')
        .pipe(gulpNgConfig('ships-web-service-url', { environment: buildConfig() }))
...
gulp.task('pre-publish', function () {
    return gulp.src('C:/inetpub/wwwroot/Ships/*', { read: false })
               .pipe(rimraf({ force: true }));
});

gulp.task('publish', ['pre-publish'], function () {
    return gulp.src(['wwwroot/**/*', '!wwwroot/{bin,bin/**}'])
        .pipe(gulp.dest('C:/inetpub/wwwroot/ShipsWeb'));
});